...
class JellyCount:   
    ...
    def do_LDA(self):
        ...
        self._compute_pca(df=self.normalised_df_20, ax_arr=ax_arr[0])
        self._compute_pca(df=self.normalised_df_50, ax_arr=ax_arr[1])
        self._compute_pca(df=self.normalised_df_100, ax_arr=ax_arr[2])
        self._compute_pca(df=self.normalised_df_1000, ax_arr=ax_arr[3])
        self._compute_pca(df=self.normalised_df_5000, ax_arr=ax_arr[4])
        plt.tight_layout()
    ...
    def _compute_pca(self, df, ax_arr):
    ...
    def investigate_LDA_kmers(self, top_kmer_n=1000):
